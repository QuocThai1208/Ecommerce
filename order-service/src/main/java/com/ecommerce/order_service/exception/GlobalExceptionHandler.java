...
public class GlobalExceptionHandler {
    final private String MIN_ATTRIBUTE = "min";
    final private String PARAM_NAME = "paramName";
    ...
    // Lỗi chuyển đổi dữ liệu kh hợp lệ
    ...
    ResponseEntity<ApiResponse> handlingMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException exception){
        ...
        ErrorCode errorCode = ErrorCode.VALUE_PARAM_INVALID;
        return ResponseEntity.status(errorCode.getStatusCode()).body(
                ApiResponse.builder()
                        .code(errorCode.getCode())
                        .message(mapParamName(errorCode.getMessage(), paramName))
                        .build());
    }

    // Param date không hợp lệ
    @ExceptionHandler(value = DateTimeParseException.class)
    ResponseEntity<ApiResponse> handlingDateTimeParseException(DateTimeParseException exception){
        ErrorCode errorCode = ErrorCode.DATE_PARAM_INVALID;
        return ResponseEntity.status(errorCode.getStatusCode()).body(
                ApiResponse.builder()
                        .code(errorCode.getCode())
                        .message(errorCode.getMessage())
    ...
    private String mapParamName(String message, String paramNameValue){
        return message.replace("{" + PARAM_NAME + "}", paramNameValue);
    }
}